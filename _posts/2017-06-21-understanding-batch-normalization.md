...
For any hidden layer, we can normalize the value of say $$ Z^{[3]} $$ so as to train the values of $$ W^{[4]}, b^{[4]} $$ faster and make our model more efficient.
...
This idea of your data distribution changing is known as $$\textbf{\textit{Covariate shift}} $$. The idea is that if you've learned some X to Y mapping $$ X \rightarrow Y $$, if at any time the distribution of $$ X $$ changes, then you might need to retrain your learning algorithm.
